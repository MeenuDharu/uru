	...
	constructor(public socket: Socket, private router: Router, public userService: UserService, public snackBar: SnackbarService, public apiService: ApiService, public commonService: CommonService, private cookieService: CookieService) {
		...
		this.socket.on("update_valet", (data) => {
			...
			if (data.vehicle_status == 'on_hold') {
				...
				this.userService.vehicle.valet_delay = delay;
				...
				this.apiService.UPDATE_VALET_DELIVERY(sendData).subscribe(result => {
					console.log('result.....', result);				
				})
				...
				let r_date: any = moment(new Date(), 'mm : ss');			
				let duration = d_date.diff(r_date, 'seconds');					
				localStorage.setItem("valet_delay",String(duration))
				this.userService.progressPerc = 100;	
				let valet_details = JSON.parse(localStorage.getItem('valet_details'));
				let sendData = {
					valet_id: valet_details.valet_id,
				...
				console.log("valet Socket send data.............", valet_details)
				this.apiService.UPDATE_VALET_DELIVERY(sendData).subscribe(result => {
					console.log('result.....', result);
					//this.router.navigate(['/bill/confirm']);
				})	
				...
				let duration = d_date.diff(r_date, 'seconds');					
				localStorage.setItem("valet_delay",String(duration))
				this.userService.vehicle.valet_delay  = duration;
				this.userService.progressPerc = 100;	
				let valet_details = JSON.parse(localStorage.getItem('valet_details'));
				let sendData = {
					valet_id: valet_details.valet_id,
				...
				// this.apiService.UPDATE_VALET_STATUS({ valet_id : this.commonService.valet_details._id, valet_status : 'vehicle_parked' }).subscribe( result => {
				//   console.log('result.....', result);
				// })
				document.getElementById("openValetStatusOpenModal").click();
				...
				document.getElementById("openValetStatusOpenModal").click();
				localStorage.removeItem('reRequest');
